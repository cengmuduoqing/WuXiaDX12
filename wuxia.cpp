	...
	if (Status == STATUS_INFO_LENGTH_MISMATCH) {
		...
		for (; ProcessInformation->NextEntryOffset; ProcessInformation = (SYSTEM_PROCESS_INFORMATION*)((unsigned __int64)ProcessInformation + ProcessInformation->NextEntryOffset)) {
			...
			if (!std::wstring(ProcessInformation->ImageName.Buffer).compare(WuXiaName) && WuXiaId.find(ProcessInformation->ProcessId) == WuXiaId.end()) {
				...
				__int8* CurrentDirectoryAddress = nullptr;
				SIZE_T RegionSize = 0x1000;
				auto Status = ZwAllocateVirtualMemory(ProcessHandle, (void**)&CurrentDirectoryAddress, 0, &RegionSize, MEM_COMMIT, PAGE_READWRITE);
				if (NT_ERROR(Status)) {
					CloseHandle(ProcessHandle);
					continue;
				}
				...
				Status = ZwWriteVirtualMemory(ProcessHandle, CurrentDirectoryAddress, CurrentDirectory, 256, &NumberOfBytesWritten);
				if (NT_ERROR(Status)) {
					CloseHandle(ProcessHandle);
					continue;
				}
				...
				std::cout << "\t在使用过程中遇到任何问题或发现潜在bug，欢迎通过GitHub Issues向我反馈\t" << std::endl;
